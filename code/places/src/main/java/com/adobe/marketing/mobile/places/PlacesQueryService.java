...
class PlacesQueryService {
			...
			connection -> {
				...
				if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
					...
					final String message = String.format(
						"Unable to get nearby places, connection failed with status %s, message %s",
				...
				try {
					...
					final JSONObject placesJson = responseJson.getJSONObject(
	...
	private List<PlacesPOI> getContainsUserPOIs(final JSONObject placesJson) throws JSONException {
		...
		final JSONArray containsUserPOIArray = placesJson.optJSONArray(
			PlacesConstants.QueryResponseJsonKeys.POI_MEMBERS
		);
	...
	private PlacesPOI createPlacesPOIFromJson(final JSONObject poiJson) {
		try {
			...
			try {
				latitude =
					Double.parseDouble(
						poiDetails.optString(POI_INDEX_LATITUDE, String.valueOf(PlacesConstants.INVALID_LAT_LON))
					);
				longitude =
					Double.parseDouble(
						poiDetails.optString(POI_INDEX_LONGITUDE, String.valueOf(PlacesConstants.INVALID_LAT_LON))
					);
				...
				Log.warning(
					PlacesConstants.LOG_TAG,
					CLASS_NAME,
					"Ignoring POI with identifier %s, exception occurred while reading latitude/ longitude",
					identifier
				);
				return null;
			}
