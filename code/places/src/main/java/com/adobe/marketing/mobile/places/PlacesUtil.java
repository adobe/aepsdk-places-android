...
public class PlacesUtil {
    ...
    /**
     * Converts the list of {@link PlacesPOI} objects to list of {@link Map} representing {@code
     * PlacesPOI}.
     *
    ...
    /**
     * Converts the list oflist of {@link Map} representing {@code PlacesPOI} into {@link PlacesPOI}
     * objects.
     *
            ...
            PlacesPOI poi =
                    new PlacesPOI(
                            ...
                            DataReader.optStringMap(
                                    eachMap, PlacesConstants.POIKeys.METADATA, null));
            poi.setUserIsWithin(
                    DataReader.optBoolean(eachMap, PlacesConstants.POIKeys.USER_IS_WITHIN, false));
            poiMapList.add(poi);
        }
        return poiMapList;
    }
    ...
    static Map<String, String> convertPOIMetadataToStringMap(@NonNull final JSONObject jsonObject) {
        ...
        while (keys.hasNext()) {
            ...
            try {
                ...
                // Metadata should only contain values that are primitive datatype, hence ignoring
                // Map or Array values
                if (value instanceof JSONObject || value instanceof JSONArray) {
                    Log.warning(
                            ...
                            CLASS_NAME,
                            String.format(
                                    "Ignoring POI metadata with key: %s which contains invalid"
                                            + " datatype.",
                                    key));
                    continue;
                }
