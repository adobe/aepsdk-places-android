...
public class PlacesExtension extends Extension {
	...
	void handlePlacesRequestEvent(@NonNull final Event event) {
		...
		if (eventData == null || eventData.isEmpty()) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"handlePlacesRequestEvent - Ignoring Places Request event, eventData is empty."
			);
			return;
		}

		final String requestType = DataReader.optString(
			eventData,
			PlacesConstants.EventDataKeys.Places.REQUEST_TYPE,
			null
		);
		if (StringUtils.isNullOrEmpty(requestType)) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"handlePlacesRequestEvent - Ignoring Places Request event due to missing request type"
			);
			return;
		}
		...
		switch (requestType) {
			...
			default:
				Log.debug(
					...
					CLASS_NAME,
					String.format(
						"handlePlacesRequestEvent - Ignoring Places Request event due to a unrecognized request type - %s.",
						requestType
					)
				);
				break;
	...
	void handleConfigurationResponseEvent(@NonNull final Event event) {
		...
		if (getMobilePrivacyStatus(configData) == MobilePrivacyStatus.OPT_OUT) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"handleConfigurationResponseEvent - Stopping Places processing due to privacy opt-out."
			);
	...
	private void handleGetUserWithinPOIsEvent(@NonNull final Event event) {
		Log.trace(
			PlacesConstants.LOG_TAG,
			CLASS_NAME,
			"handleGetUserWithinPOIsEvent - Handling get user-within points of interest event."
		);
	...
	private void handleGetLastKnownLocation(@NonNull final Event event) {
		...
		placesDispatcher.dispatchLastKnownLocation(location.getLatitude(), location.getLongitude(), event);
		// dispatch lastKnownLocation for all other listeners
		placesDispatcher.dispatchLastKnownLocation(location.getLatitude(), location.getLongitude(), null);
	}
	...
	private void saveLastKnownLocation(@NonNull final Event event) {
		...
		if (getMobilePrivacyStatus(configData) == MobilePrivacyStatus.OPT_OUT) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"saveLastKnownLocation - Ignoring to save the last known location, Privacy opted out"
			);
			return;
		}

		final Map<String, Object> eventData = event.getEventData();
		final double latitude = DataReader.optDouble(
			eventData,
			PlacesConstants.EventDataKeys.Places.LATITUDE,
			PlacesConstants.INVALID_LAT_LON
		);
		final double longitude = DataReader.optDouble(
			eventData,
	...
	private void handleSetAuthorizationStatusEvent(@NonNull final Event event) {
		...
		final String authStatusString = DataReader.optString(
			data,
			PlacesConstants.EventDataKeys.Places.AUTH_STATUS,
			null
		);
	...
	private void handleGetNearByPlaceEvent(@NonNull final Event event, final Map<String, Object> configData) {
		...
		if (!placesConfig.isValid()) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"handleGetNearByPlaceEvent - Ignoring the get nearby places event, Invalid Configuration"
			);
			placesDispatcher.dispatchNearbyPlaces(new ArrayList<>(), PlacesRequestError.CONFIGURATION_ERROR, event);
			return;
		}
		...
		if (getMobilePrivacyStatus(configData) == MobilePrivacyStatus.OPT_OUT) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"handleGetNearByPlaceEvent - Ignoring the get nearby places event, Privacy opted out."
			);
			placesDispatcher.dispatchNearbyPlaces(new ArrayList<>(), PlacesRequestError.PRIVACY_OPTED_OUT, event);
			return;
		}
		...
		queryService.getNearbyPlaces(
			...
			response -> {
				if (!response.isSuccess) {
					...
					placesDispatcher.dispatchNearbyPlaces(new ArrayList<>(), response.resultStatus, event);
					return;
				}
				...
				placesDispatcher.dispatchNearbyPlaces(response.getAllPOIs(), PlacesRequestError.OK, event);
	...
	private MobilePrivacyStatus getMobilePrivacyStatus(final Map<String, Object> configData) {
		if (
			...
			final String privacyString = DataReader.optString(
				configData,
