...
class PlacesDispatcher {
	...
	void dispatchNearbyPlaces(final List<PlacesPOI> poiList, final PlacesRequestError resultStatus, final Event event) {
		...
		if (event != null) {
			Log.debug(
				...
				CLASS_NAME,
				"dispatchNearbyPlaces - Dispatching nearby places response event for `getNearbyPointsOfInterest` API callback with %d POIs",
				poiList.size()
			);
			...
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"dispatchNearbyPlaces - Dispatching nearby places response event for all other listeners with %d POIs",
				poiList.size()
			);
			final Event responseEvent = new Event.Builder(
				PlacesConstants.EventName.RESPONSE_GETNEARBYPLACES,
	...
	void dispatchRegionEvent(final PlacesRegion region) {
		...
		final Event regionEvent = new Event.Builder(
	...
	void dispatchUserWithinPOIs(final List<PlacesPOI> poiList, final Event event) {
		...
		if (event != null) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"dispatchUserWithinPOIs - Dispatching user within POIs event for `getCurrentPointsOfInterest` API callback with %d POIs",
				poiList.size()
			);
			final Event responseEvent = new Event.Builder(
			...
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"dispatchUserWithinPOIs - Dispatching user within POIs event for other listeners with %d POIs",
				poiList.size()
			);
			final Event responseEvent = new Event.Builder(
				PlacesConstants.EventName.RESPONSE_GETUSERWITHINPLACES,
	...
	void dispatchLastKnownLocation(final double latitude, final double longitude, final Event event) {
		...
		if (event != null) {
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"dispatchLastKnownLocation - Dispatching last known location event for `getLastKnownLocation` API callback with latitude: %s and longitude: %s",
				latitude,
				longitude
			);
			final Event responseEvent = new Event.Builder(
			...
			Log.debug(
				PlacesConstants.LOG_TAG,
				CLASS_NAME,
				"dispatchLastKnownLocation - Dispatching last known location event for other listeners with latitude: %s and longitude: %s",
				latitude,
				longitude
			);
			final Event responseEvent = new Event.Builder(
	...
	void dispatchExperienceEventToEdge(@NonNull final PlacesRegion regionEvent) {
		...
		final Event experienceEvent = new Event.Builder(
