...
public class PlacesTests {
	...
	private ArgumentCaptor<AdobeCallbackWithError> callbackCaptor = ArgumentCaptor.forClass(
	...
	public void getNearbyPlaces_should_dispatchPlacesRequestContentEvent() {
		...
		mockedMobileCore.verify(() ->
			MobileCore.dispatchEventWithResponseCallback(eventCaptor.capture(), anyLong(), any())
		);
		...
		assertEquals(
			...
			22.22,
			(double) dispatchedEvent.getEventData().get(PlacesTestConstants.EventDataKeys.Places.LATITUDE),
			0.0
		);
		assertEquals(
			"event has the correct longitude data",
			-11.11,
			(double) dispatchedEvent.getEventData().get(PlacesTestConstants.EventDataKeys.Places.LONGITUDE),
			0.00d
		);
		assertEquals(
			"event has the correct places count",
			20,
			(int) dispatchedEvent.getEventData().get(PlacesTestConstants.EventDataKeys.Places.PLACES_COUNT),
			0
		);
		assertEquals(
	...
	public void getNearbyPlaces_when_responseEvent_with_validPOIs() {
		...
		Event responseEvent = new Event.Builder("responseEvent", EventType.PLACES, EventSource.RESPONSE_CONTENT)
		...
		Places.getNearbyPointsOfInterest(mockLocation(22.22, -11.11), 20, successCallback, errorCallback);
		mockedMobileCore.verify(() ->
			MobileCore.dispatchEventWithResponseCallback(any(), anyLong(), callbackCaptor.capture())
		);
	...
	public void getLastKnownLocation_when_responseEvent_with_InvalidLatLong() {
		...
		Event responseEventInvalidLatLong = new Event.Builder(
			"responseEvent",
			EventType.PLACES,
	...
	public void processGeofenceEvent_should_dispatchPlacesRequestContentEvent() {
		...
		Places.processGeofenceEvent(
			mockGeofencingEvent(Geofence.GEOFENCE_TRANSITION_ENTER, Arrays.asList(mockGeofence1()))
		);
	...
	public void processGeofenceEvent_when_multiplePOIExited() {
		// test
		Places.processGeofenceEvent(
			mockGeofencingEvent(Geofence.GEOFENCE_TRANSITION_EXIT, Arrays.asList(mockGeofence1(), mockGeofence2()))
		);
	...
	public void test_clear_dispatchPlacesRequestContentEvent() {
		...
		// verify dispatched event details
		assertEquals("event has correct name", PlacesTestConstants.EventName.REQUEST_RESET, dispatchedEvent.getName());
		assertEquals("event has correct event type", EventType.PLACES, dispatchedEvent.getType());
		assertEquals("event has correct event source", EventSource.REQUEST_CONTENT, dispatchedEvent.getSource());
		assertEquals(
			"event has the correct request type",
