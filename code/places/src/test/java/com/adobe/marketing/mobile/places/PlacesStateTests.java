...
public class PlacesStateTests {
	...
	public void GetPlacesSharedState() {
		...
		assertEquals(
			"currentPOI",
			((Map<?, ?>) data.get(PlacesTestConstants.SharedStateKeys.CURRENT_POI)).get(
					PlacesTestConstants.POIKeys.IDENTIFIER
				)
		);
		assertEquals(
			"lastEnteredPOI",
			((Map<?, ?>) data.get(PlacesTestConstants.SharedStateKeys.LAST_ENTERED_POI)).get(
					PlacesTestConstants.POIKeys.IDENTIFIER
				)
		);
		assertEquals(
			"lastExitedPOI",
			((Map<?, ?>) data.get(PlacesTestConstants.SharedStateKeys.LAST_EXITED_POI)).get(
					PlacesTestConstants.POIKeys.IDENTIFIER
				)
		);
	...
	public void loadLastKnownLocation_when_validLocation() {
		...
		MockedConstruction<Location> mockedLocationConstruction = mockConstruction(
			Location.class,
			(mock, context) -> {
	...
	private LinkedHashMap<String, PlacesPOI> getPersistedCachedPOI() throws JSONException {
		...
		verify(placesDataStore, times(1))
			.setString(eq(PlacesTestConstants.DataStoreKeys.NEARBYPOIS), persistenceValueCaptor.capture());
	...
	private PlacesPOI getPersistedLastEnteredPOI() throws JSONException {
		ArgumentCaptor<String> persistenceValueCaptor = ArgumentCaptor.forClass(String.class);
		verify(placesDataStore, times(1))
			.setString(eq(PlacesTestConstants.DataStoreKeys.LAST_ENTERED_POI), persistenceValueCaptor.capture());
	...
	private PlacesPOI getPersistedLastExitedPOI() throws JSONException {
		ArgumentCaptor<String> persistenceValueCaptor = ArgumentCaptor.forClass(String.class);
		verify(placesDataStore, times(1))
			.setString(eq(PlacesTestConstants.DataStoreKeys.LAST_EXITED_POI), persistenceValueCaptor.capture());
		return new PlacesPOI((persistenceValueCaptor.getValue()));
	}
	...
	private PlacesPOI getPersistedCurrentPOI() throws JSONException {
		ArgumentCaptor<String> persistenceValueCaptor = ArgumentCaptor.forClass(String.class);
		verify(placesDataStore, times(1))
			.setString(eq(PlacesTestConstants.DataStoreKeys.CURRENT_POI), persistenceValueCaptor.capture());
		return new PlacesPOI((persistenceValueCaptor.getValue()));
	}
	...
	private String getPersistedPermissionStatus() {
		ArgumentCaptor<String> persistenceValueCaptor = ArgumentCaptor.forClass(String.class);
		verify(placesDataStore, times(1))
			.setString(eq(PlacesTestConstants.DataStoreKeys.AUTH_STATUS), persistenceValueCaptor.capture());
	...
	private long getPersistedMembershipValidUntilTimestamp() {
		...
		verify(placesDataStore, times(1))
			.setLong(eq(PlacesTestConstants.DataStoreKeys.MEMBERSHIP_VALID_UNTIL), persistenceValueCaptor.capture());
		return persistenceValueCaptor.getValue();
	}
	...
	private PlacesQueryResponse GetSampleSuccessPlacesResponse(
		final int containsUserPOICount,
		...
		for (int i = 0; i < containsUserPOICount; i++) {
			response.containsUserPOIs.add(
				new PlacesPOI("containsUserPOI " + i, "hidden", 34.33, -121.55, 150, "libraryName", 2, null)
			);
		}
		...
		for (int i = 0; i < nearByPOIsCount; i++) {
			response.nearByPOIs.add(
				new PlacesPOI("nearByPOI" + i, "hidden", 34.33, -121.55, 150, "libraryName", 2, null)
			);
		}
