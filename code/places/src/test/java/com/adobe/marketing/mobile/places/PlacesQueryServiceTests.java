...
public class PlacesQueryServiceTests {
    ...
    @Mock private HttpConnecting connecting;

    @Mock private PlacesConfiguration configuration;
    ...
    public void getNearByPlaces_when_EventDataWithNoLatitude() {
        ...
        queryService.getNearbyPlaces(
                ...
                validConfiguration(),
                response -> {
                    // since we are mocking network layer, the callback executes synchronously and
                    // the assertions are executed.
    ...
    public void getNearByPlaces_happy_makesNetworkRequest() {
        ...
        ArgumentCaptor<NetworkRequest> networkResponseCapture =
        ...
        assertNull(
                "network request has the correct request property",
                networkResponseCapture.getValue().getHeaders());
        assertNull(
                "network request has the correct connect payload",
                networkResponseCapture.getValue().getBody());
        assertEquals(
    ...
    public void getNearByPlaces_when_HttpNot200OK() {
        ...
        queryService.getNearbyPlaces(
                ...
                response -> {
                    ...
                    assertEquals(
                            "has the correct error message",
                            "Unable to get nearby places, connection failed with status 400,"
                ...
                response -> {
                    ...
                    assertTrue(
                            ...
                            response.errorMessage.contains(
                                    "Unable to get nearby places, Failed with exception:"
        ...
        when(connecting.getInputStream())
                .thenReturn(
                        new ByteArrayInputStream(
                                responseWithInvalidKey().getBytes(StandardCharsets.UTF_8)));
        doAnswer(
                        invocation -> {
                            ((NetworkCallback) invocation.getArguments()[1]).call(connecting);
                            return null;
        ...
        queryService.getNearbyPlaces(
                ...
                response -> {
                    ...
                    assertEquals(
                            ...
                            response.nearByPOIs.get(0).getIdentifier());
                    assertEquals(
                            "Marriott Marquis - Street Level",
                            response.nearByPOIs.get(0).getName());
                    ...
                    assertEquals(
                            ...
                            response.containsUserPOIs.get(0).getIdentifier());
